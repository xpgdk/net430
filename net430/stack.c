...
static const uint8_t solicited_mcast_prefix[] = {0xFF,0x02, 0x00,0x00, 0x00,0x00, 0x00,0x00, 0x00,0x00, 0x00,0x01, 0xFF, 0x00};
static const uint8_t unspec_addr[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
static const uint8_t all_router_mcast[] = {0xFF, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02};
...
void send_neighbor_solicitation(const uint8_t *dst_mac, const uint8_t *src_addr, const uint8_t *dst_addr, const uint8_t *addr);
void send_neighbor_advertisment(const uint8_t *dst_mac, const uint8_t *src_addr, const uint8_t *dst_addr, const uint8_t *addr);
void send_router_solicitation(const uint8_t *src_addr, const uint8_t *dst_addr);
void construct_solicited_mcast_addr(uint8_t *solicited_mcast, const uint8_t *addr);
void assign_address_from_prefix(uint8_t *ipv6_addr, uint8_t prefixLength);

void net_send_icmp(uint8_t type, uint8_t code, uint8_t *body, uint16_t body_length);
void net_send_echo_reply(uint16_t id, uint16_t seqNo);
bool has_ipv6_addr(const uint8_t *ipaddr);

void
assign_address_from_prefix(uint8_t *addr, uint8_t prefixLength)
{
	if( prefixLength != 64) {
		uart_puts("Unsupported prefix length for Ethernet\n");
		return;
	}

	memcpy(ipv6_addr, addr, prefixLength / 8);
	memcpy(ipv6_addr+prefixLength/8, eui64, 8);
	uart_puts("IPv6 Address configured: ");
	print_addr(ipv6_addr);
}

void
register_mac_addr(const uint8_t *mac, const uint8_t *addr) {
	if (has_ipv6_addr(addr)) {
		return;
	}
	//mem_read(addr_map_id, addt_map_next*sizeof(struct addr_map_entry), &e, count);
	mem_write(addr_map_id, addr_map_next*sizeof(struct addr_map_entry), mac, 6);
	mem_write(addr_map_id, addr_map_next*sizeof(struct addr_map_entry)+6, addr, 16);
	...
	uart_puts("Stored address in cache:");
	print_addr(addr);
	print_buf(mac, 6);
...
bool
find_mac_addr(uint8_t *mac, const uint8_t *addr) {
	for(int i=0;i<ADDR_MAP_SIZE; i++) {
		uint8_t addr_map[16];
		mem_read(addr_map_id, i*sizeof(struct addr_map_entry)+6, addr_map, 16);
...
bool
has_ipv6_addr(const uint8_t *ipaddr) {
	for(int i=0;i<ADDR_MAP_SIZE; i++) {
		uint8_t addr_map[16];
		mem_read(addr_map_id, i*sizeof(struct addr_map_entry)+6, addr_map, 16);
...
bool
is_mac_equal(const uint8_t *mac1, const uint8_t *mac2) {
	for(int i=0; i<6; i++) {
		if( mac1[i] != mac2[i] ) {
			return false;
		}
...
void
net_init(const uint8_t *mac) {
#ifdef HAVE_TCP
	tcp_init();
#endif
	memset(ipv6_addr, 0x00, 16);
	uart_puts("MEM FREE:");
	uart_puthex(mem_free());
	uart_puts("\r\n");
	...
	send_neighbor_solicitation(ether_bcast, unspec_addr, addr_solicited, addr_link);

	net_state = STATE_DAD;
}

void
construct_solicited_mcast_addr(uint8_t *solicited_mcast, const uint8_t *addr)
{
...
void
net_start_ipv6_packet(struct ipv6_packet_arg *arg) {
	/* IPv6 Header Format:
...
void net_end_ipv6_packet() {
	...
	if( address_lookup != NULL) {
		...
		uart_puts("\r\n");
		send_neighbor_solicitation(ether_bcast, addr_link, address_lookup, address_lookup);
		net_state = STATE_WAITING_ADVERTISMENT;
	}
}

void
net_send_icmp(uint8_t type, uint8_t code, uint8_t *body, uint16_t body_length) {
	/**
	  +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
	  |     Type      |     Code      |          Checksum             |
	  +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
...
void
send_neighbor_solicitation(const uint8_t *dst_mac, const uint8_t *src_addr, const uint8_t *dst_addr, const uint8_t *addr) {
...
void
send_neighbor_advertisment(const uint8_t *dst_mac, const uint8_t *src_addr, const uint8_t *dst_addr, const uint8_t *addr) {
	/*
	   +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
...
void
send_router_solicitation(const uint8_t *src_addr, const uint8_t *dst_addr)
{
...
net_tick(void) {
	switch(net_state) {
		case STATE_DAD:
			...
			uart_puts("IPv6 Link Local address: ");
			print_addr(addr_link);
...
void
calc_checksum(const uint8_t *buf, uint16_t count) {
...
void
handle_ethernet(struct etherheader *header,
	uint16_t length,
	DATA_CB dataCb,
	void *priv)
{
...
void
handle_ipv6(uint8_t *macSource, uint16_t length, DATA_CB dataCb, void *priv)
{
	uint16_t count;
	uint8_t buf[2];
	...
	DPRINTADDR_IPV6((sourceAddr));
	DPRINTADDR_IPV6((destAddr));
	...
	} else if(destAddr[0] == 0xFF && destAddr[1] == 0x02 && destAddr[15] == 0x1) {
		receive = true;
	...
	switch(nextHeader) {
		case PROTO_ICMP:
			handle_icmp(macSource, sourceAddr, destination, payload_length, dataCb, priv);
			break;
		case PROTO_UDP:
			handle_udp(macSource, sourceAddr, destination, payload_length, dataCb, priv);
			break;
#ifdef HAVE_TCP
		case PROTO_TCP:
...
void
print_buf(const uint8_t *data, unsigned int count) {
	char buf[10];
...
void
handle_icmp(uint8_t *macSource, uint8_t *sourceAddr, uint8_t *destIPAddr, uint16_t length, DATA_CB dataCb, void *priv) {
	...
	DPRINTF_ICMP(("Checksum: "));
	DPRINTBUF_ICMP((payload+2, 2));
	...
	if( net_state != STATE_IDLE && type != ICMP_TYPE_NEIGHBOR_SOLICITATION && type != ICMP_TYPE_NEIGHBOR_ADVERTISMENT)
		return;
	...
	switch(type) {
			...
			//if( memcmp(payload+4, addr_link, 16) == 0) {
				...
				if( length > 20 ) {
					...
					if( payload[20] == 0x01 ) {
						...
						send_neighbor_advertisment(payload+22, payload+4, sourceAddr, payload+4);
					}
				}
		...
		case ICMP_TYPE_ROUTER_ADVERTISMENT:
			 ...
			 */
			{
				...
				uart_puts("Got router advertisment\n");
